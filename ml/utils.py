...
def visualize_boxes(image_path, person_boxes, danger_zones):
    ...
    for person in polygons_person:
        for zone in polygons_zone:
            ...
            if distance != 0:
                person_point, zone_point = nearest_points(person, zone)
                x1 = int(person_point.coords[0][0])
                y1 = int(person_point.coords[0][1])
                x2 = int(zone_point.coords[0][0])
                y2 = int(zone_point.coords[0][1])
                cv2.line(image, (x1, y1), (x2, y2), (255, 50, 50), 3)
                cv2.putText(image, str(round(distance)), (((x1 + x2) // 2) + 5, ((y1 + y2) // 2) + 5), cv2.FONT_HERSHEY_SIMPLEX, 3,
                            (0, 0, 0), 3, cv2.LINE_AA)
